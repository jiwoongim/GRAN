...
class RecGenI128(object):

    def __init__(self, model_params, nkerns=[1,1,8,4,2,1], ckern=172, filter_sizes=[4,5,5,5,5,5]):
        ...
        self.L6 = Deconv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[5], bnkern=self.nkerns[4] , bfilter_sz=filter_sizes[4], tfilter_sz=filter_sizes[5])

        self.L7 = Conv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[1], bnkern=self.nkerns[0] , bfilter_sz=filter_sizes[0], tfilter_sz=filter_sizes[1])
        self.L8 = BN_Conv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[2], bnkern=self.nkerns[1] , bfilter_sz=filter_sizes[1], tfilter_sz=filter_sizes[2])
        self.L9 = BN_Conv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[3], bnkern=self.nkerns[2] , bfilter_sz=filter_sizes[2], tfilter_sz=filter_sizes[3])
        self.L10 = BN_Conv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[4], bnkern=self.nkerns[3] , bfilter_sz=filter_sizes[3], tfilter_sz=filter_sizes[4])
        self.L11 = BN_Conv_layer(self.batch_sz, numpy_rng, tnkern=self.nkerns[5], bnkern=self.nkerns[4] , bfilter_sz=filter_sizes[4], tfilter_sz=filter_sizes[5])
        ...
        self.params = self.L0a.params + self.L0b.params + self.L1.params +   \
                        ...
                        + self.L8.params +self.L9.params + self.L10.params +self.L11.params
    ...
    def forward(self, Z, H_Ct, testF=False, atype='relu'):
        ...
        H5 = self.L5.deconv(H4, atype=atype)
        H6 = self.L6.deconv(H5, atype='linear') 
    ...
    def backward(self,Ct, atype='relu', testF=False):
        ...
        """
        H0 = self.L11.conv(Ct, atype=atype)
        H1 = self.L10.conv(H0, atype=atype)
        H2 = self.L9.conv(H1, atype=atype) 
        H3 = self.L8.conv(H2, atype=atype) 
        H4 = self.L7.conv(H3, atype=atype) 
        H4 = H4.flatten(2)
        
        return self.L0b.propagate(H4, testF=testF, atype='tanh')
    ...
    def apply_recurrence(self, num_steps, Zs, H_Ct, scanF=True):
        ...
        if scanF:
            [H_Ct, C], updates = theano.scan(fn=recurrence, outputs_info=[H_Ct, None], \
                        sequences=[T.arange(num_steps, dtype='int16')], non_sequences=[Zs])
        else:
    ...
    def weight_decay_l2(self):
        ...
        return 0.5*((self.L2.W **2).sum() \
                + (self.L3.W**2).sum() + (self.L4.W**2).sum() + (self.L5.W**2).sum())
